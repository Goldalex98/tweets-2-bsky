...
Current release: app `3.3.3`, schema **v7** (`Source → Route → Destination → BlueskyAccount`), managed Bluesky
...
1. **Fetch sweep** — enabled sources are checked on the scheduler interval through a global X rate limiter. New posts are written to `post_queue` in SQLite under the data directory.
