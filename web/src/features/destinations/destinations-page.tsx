...
function FolderButton({ active, onClick, emoji, name }: { active: boolean; onClick(): void; emoji: string; name: string }) {
  return <button type="button" aria-pressed={active} onClick={onClick} className={cn('flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm font-medium transition-colors focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background', active ? 'bg-foreground text-background' : 'text-muted-foreground hover:bg-muted hover:text-foreground')}><span aria-hidden="true">{emoji}</span><span className="truncate">{name}</span><Folder aria-hidden="true" className="ml-auto h-3.5 w-3.5 opacity-60" /></button>;
}
